...
int binaryTreeType<elemType>::height(binaryTreeNode<elemType>* p) const
{
    if (p == NULL)
        ...
        return 1 + max(height(p->llink), height(p->rlink)); //add left subtree height if it is greater
}                                                           //add right subtree height if it is greater
    ...
    binaryTreeNode<elemType>* otherTreeRoot)
{
    ...
    else
    {
        ...
        copyTree(copiedTreeRoot->llink, otherTreeRoot->llink);  //recursively copy left subtree
        copyTree(copiedTreeRoot->rlink, otherTreeRoot->rlink);  //recursively copy right subtree
    }
} //end copyTree
...
void binaryTreeType<elemType>::inorder(binaryTreeNode<elemType>* p) const
{
    if (p != NULL)
    {
        inorder(p->llink);          //left link
        cout << p->info << " ";     //node
        inorder(p->rlink);          //right link
    }
}
...
void binaryTreeType<elemType>::preorder(binaryTreeNode<elemType>* p) const
{
    ...
    {
        cout << p->info << " ";     //node
        preorder(p->llink);         //left link
        preorder(p->rlink);         //right link
    }
}
...
void  binaryTreeType<elemType>::destroy(binaryTreeNode<elemType>*& p)
{
    if (p != NULL)
    {
        destroy(p->llink);  //delete left subtree nodes
        destroy(p->rlink);  //delete right subtree nodes
...
int binaryTreeType<elemType>::leavesCount(binaryTreeNode<elemType>* p) const
{
    if (p == NULL)
        return 0;
    else if (p->llink == NULL && p->rlink == NULL)  //if no children
        return 1;
    else
